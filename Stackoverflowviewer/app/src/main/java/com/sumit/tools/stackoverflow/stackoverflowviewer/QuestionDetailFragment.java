...
public class QuestionDetailFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments().containsKey(ARG_ITEM_ID)) {
            ...
            mItem = StackOverflowQuestions.ITEM_MAP.get(getArguments().getString(ARG_ITEM_ID));

            Activity activity = this.getActivity();
            CollapsingToolbarLayout appBarLayout = (CollapsingToolbarLayout) activity.findViewById(R.id.toolbar_layout);
            if (appBarLayout != null) {
                appBarLayout.setTitle(mItem.question_id);
            }
            FloatingActionButton floatingActionButton = (FloatingActionButton) getActivity().findViewById(R.id.fab);
            floatingActionButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Uri uriUrl = Uri.parse(mItem.link);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        TextView rootView = (TextView) inflater.inflate(R.layout.question_detail, container, false);

        if (mItem != null) {
            rootView.setClickable(true);
            rootView.setMovementMethod(LinkMovementMethod.getInstance());
            ((TextView) rootView.findViewById(R.id.question_detail)).setText(mItem.title);
        }
